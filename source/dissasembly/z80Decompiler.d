...
string toAsm(DecompilerUnit unit){
    ...
    {
        scope (exit) location += line.getSize;
...
size_t possibleAsciiTest(const(ubyte[]) ub, size_t index){
    ...
    while (index < ub.length){
        ...
        if(isASCII(possible) || possible == 0){
            correctCount++;
            if (possible == 0) return correctCount;
        }else 
            return 0;
    }
    return 0;
...
DecompLine parseZ80Line(ref DecompilerUnit unit, const(ubyte[]) data, ref size_t index){
    static size_t lastAsciiEnd = 0;

    DecompLine line;
    line.location = index;
    size_t oldIndex = index;
    size_t asciiTest = possibleAsciiTest(data, index);
    if (asciiTest > 1 && lastAsciiEnd != index + asciiTest){
        lastAsciiEnd = index + asciiTest;
        DecompLine noteLine;
        noteLine.lineVarity = LineVarity.AssemblersNote;
        string note = "\tPossible " ~ asciiTest.to!string ~ " byte string: \"" ~ escapeString(data[index..index+asciiTest]) ~ "\"";
        noteLine.data = cast(ubyte[]) note;
        unit.lines ~= noteLine;
    }
    
    auto instruction = getInstruction_nullable(data, index);
    if (instruction == null && asciiTest > 1){
        index = oldIndex;
        line.lineVarity = LineVarity.Data;
        line.isAsciiData = true;
        line.data = new ubyte[asciiTest];
        line.data[0..$] = cast(ubyte[])data[index..index+=asciiTest];
        lastAsciiEnd = index + asciiTest;
        return line;
    }
    ...
    if (line.asmInstruction.type == InstructionType.Rst && line.asmInstruction.operands[0].rst == 0x28){
    ...
    foreach (operand ; line.asmInstruction.operands){
        ...
        }else if(operand.variety == OperandVariety.Imm16 && operand.imm16 >= unit.org && operand.imm16 < unit.org + unit.data_size_limit){
            unit.attachLabel(operand.imm16, line);
        }
    }
